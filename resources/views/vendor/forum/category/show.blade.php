                    ...
                    @if (!$category->threadsPaginated->isEmpty())
                        @foreach ($category->threadsPaginated as $thread)
                            <tr class="{{ $thread->trashed() ? "deleted" : "" }}">
                                <td>{{ $thread->like_count or 0 }}&nbsp;
                                    <a href="{{action('LikeController@anyToggleLike',['likeable'=>$thread,'likeable_type'=>'thread'])}}" title="Like this.">&hearts;</a>@if(count($thread->likes)):@endif
                                    <ul class="list-inline csv">
                                        @foreach($thread->likes as $eachLike)
                                            <li>{{ $eachLike->user->name }}</li>
                                        @endforeach
                                    </ul>
                                </td>
                                <td>
