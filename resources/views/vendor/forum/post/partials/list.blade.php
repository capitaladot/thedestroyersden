<tr id="post-{{ $post->id }}" class="{{ $post->trashed() ? 'deleted' : '' }}">
    <td>{{ $post->like_count or 0 }}&nbsp;
        <a href="{{action('LikeController@anyToggleLike',['likeable'=>$post,'likeable_type'=>'post'])}}" title="Like this.">&hearts;</a>:
        <ul class="list-inline">
            @foreach($post->likes as $eachLike)
