...
Form::macro('dateTime',function($input,$properties = [],$value = null){
	$dateArray = [0=>false,1=>false,2=>false,3=>false,4=>false];
	if(!is_null($value)){
	...
	. Form::label('Date')
	. Form::selectRange($input.'[D]',1,31,$dateArray[2] ? $dateArray[2] : date('j'),[
			$properties['notNull'] ? 'required' : ''
	])
	. Form::label('Hour')
	. Form::selectRange($input.'[H]',0,23,$dateArray[3] ? $dateArray[3] : 17,[
			$properties['notNull'] ? 'required' : ''
	]);
});
